...
Le système prompt établit les règles et cadre de l'interaction avec le modèle, transformant GPT en chatbots spécialisés adaptés à diverses situations.
...
- **Avantages** : Utile lorsque le modèle a besoin d'un contexte minimal pour produire des réponses plus précises.
- **Exemple** :
...
- **Avantages** : Particulièrement efficace pour les tâches complexes ou spécifiques où des exemples multiples aident à établir un schéma clair.
- **Exemple** :
...
- **Avantages** : Idéal pour les problèmes nécessitant plusieurs étapes de raisonnement ou une explication détaillée. Cela rend également les réponses plus interprétables.
...
- [The Big Prompt Library](https://github.com/0xeb/TheBigPromptLibrary) list une grande variété de promptes utiles
...
**Raisonnement**: Pousse le modèle à réfléchir (générer du texte) avant de répondre. Cela s'appel du **Chain of thought** (CoT).
...
Pour éviter d'écrire des promptes (message) systématiquement, créer des systèmes promptes permet de se simplifier la tâche.

**Nous allons faire une exemple de dictionnaire des synonymes:**

- **But:** Lorsque nous écrivons un mot le modèle nous renvoie une lsite de synonymes
- **utilité:** Évite de devoir réécrire le prompte à chaque fois

## Astuce pour un system prompt

Un peu de conseil de prompting:
...
6. Les llms sont bon pour repérer les structures: **Utilisez des caractères spéciaux comme `{}` pour améliiorer la structure**
7. Les llms donnerons toujours une réponse même lorsqu'ils ne savent pas: **Donnez des instruction lorsque le modèle ne sait pas**

## Exercices

- Créer une application de correction de text et de synthaxe
- Créer une application de traduction
- Créer une applicaiton pour répondre aux emails
